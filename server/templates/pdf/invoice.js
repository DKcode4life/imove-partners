function render({ invoice_number, customer_name, items = [], subtotal, tax_amount, total, due_date, notes }) {
...
<body style="font-family:Arial,sans-serif;color:#1e293b;max-width:800px;margin:0 auto;padding:40px">
  ...
  <div style="display:flex;justify-content:space-between;margin-bottom:24px">
    <div>
      ...
      <p style="margin:0;color:#64748b">Ref: ${invoice_number}</p>
    </div>
    <div style="text-align:right">
      <p style="margin:0"><strong>Customer:</strong> ${customer_name}</p>
      ${due_date ? `<p style="margin:0;color:#dc2626"><strong>Due:</strong> ${due_date}</p>` : ''}
    </div>
  </div>
  ...
  <div style="text-align:right;margin-bottom:24px">
    ...
    <p style="margin:4px 0;font-size:18px">Total Due: <strong style="color:#dc2626">£${(total || 0).toFixed(2)}</strong></p>
  </div>
  ...
  <div style="background:#eff6ff;padding:16px;border-radius:8px;margin-bottom:24px">
    ...
    <p style="margin:4px 0;font-size:14px;color:#475569">Bank: [Your Bank Name]</p>
    <p style="margin:4px 0;font-size:14px;color:#475569">Sort Code: [XX-XX-XX]</p>
    <p style="margin:4px 0;font-size:14px;color:#475569">Account: [XXXXXXXX]</p>
    <p style="margin:4px 0;font-size:14px;color:#475569">Reference: ${invoice_number}</p>
  </div>
