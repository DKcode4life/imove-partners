...
router.post('/jobs/:id/invoices/:invoiceId/send-receipt', wrap(async (req, res) => {
  ...
  if (invoice.status !== 'paid') {
    // Self-heal: the QuoteBuilder "paid" tick lives in the job's quote_state.
    // Jobs ticked before the quote-state save settled invoices (or where that
    // save was missed) have balancePaid/depositPaid set while the invoice row
    // still says 'sent' — honour the tick now instead of bouncing the receipt.
    const qs = invoice.job?.quote_state || {};
    const tickedPaid = invoice.invoice_type === 'main'
      ? qs.balancePaid === true
    ...
    const dateStr = invoice.invoice_type === 'main' ? qs.balancePaidDate : qs.depositPaidDate;
    const paidAt = dateStr ? new Date(dateStr) : new Date();
    const settled = await settleInvoiceFromQuoteState(prisma, { jobId, invoiceId, paidAt });
    if (!settled) {
      return res.status(400).json({ error: 'Cannot send receipt — invoice is not yet marked paid' });
    }
    invoice = { ...invoice, ...settled };
  }
